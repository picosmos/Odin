...
public class BasicAuthMiddleware(RequestDelegate next, ILogger<BasicAuthMiddleware> logger)
{
    ...
    private static readonly string USERNAME = Environment.GetEnvironmentVariable("ODIN_AUTH_USERNAME") ?? "admin";
    private static readonly string PASSWORD = Environment.GetEnvironmentVariable("ODIN_AUTH_PASSWORD") ?? "password";
